...
class clsOpenTBS extends clsTbsZip
{
	...
	function OnInstall()
	{
		...
		if (!isset($TBS->OtbsAutoLoad))
			$TBS->OtbsAutoLoad = true; // TBS will load the subfile regarding to the extension of the archive
		if (!isset($TBS->OtbsConvBr))
			$TBS->OtbsConvBr = false; // string for NewLine conversion
		if (!isset($TBS->OtbsAutoUncompress))
			$TBS->OtbsAutoUncompress = $this->Meth8Ok;
		if (!isset($TBS->OtbsConvertApostrophes))
			$TBS->OtbsConvertApostrophes = true;
		if (!isset($TBS->OtbsSpacePreserve))
			$TBS->OtbsSpacePreserve = true;
		if (!isset($TBS->OtbsClearWriter))
			$TBS->OtbsClearWriter = true;
		if (!isset($TBS->OtbsClearMsWord))
			$TBS->OtbsClearMsWord = true;
		if (!isset($TBS->OtbsMsExcelConsistent))
			$TBS->OtbsMsExcelConsistent = true;
		if (!isset($TBS->OtbsMsExcelExplicitRef))
			$TBS->OtbsMsExcelExplicitRef = true;
		if (!isset($TBS->OtbsClearMsPowerpoint))
			$TBS->OtbsClearMsPowerpoint = true;
		if (!isset($TBS->OtbsGarbageCollector))
			$TBS->OtbsGarbageCollector = true;
		if (!isset($TBS->OtbsMsExcelCompatibility))
	...
	function BeforeLoadTemplate(&$File, &$Charset)
	{
		...
		$TBS =& $this->TBS;
		if ($TBS->_Mode != 0)
			return; // If we are in subtemplate mode, the we use the TBS default process
		...
		if (($SubFileLst !== '') && ($SubFileLst !== false)) {
			if (is_string($SubFileLst))
				$SubFileLst = explode(';', $SubFileLst);
	...
	function BeforeShow(&$Render, $File = '')
	{
		...
		switch ($this->ExtEquiv) {
			case 'ods':
				$this->OpenDoc_SheetSlides_DeleteAndDisplay(true);
				break;
			case 'odp':
				$this->OpenDoc_SheetSlides_DeleteAndDisplay(false);
				break;
			case 'xlsx':
				$this->MsExcel_SheetDeleteAndDisplay();
				break;
			case 'pptx':
				$this->MsPowerpoint_SlideDelete();
				break;
		}
	...
	function TbsLoadSubFileAsTemplate($SubFileLst)
	{
		...
		if (!is_array($SubFileLst))
			$SubFileLst = array($SubFileLst);
		...
		foreach ($SubFileLst as $SubFile) {
			...
			if ($idx === false) {
				...
				$this->TbsStorePark();
				// Load the subfile
				if (!is_string($SubFile))
					$SubFile = $this->TbsGetFileName($idx);
	...
	function TbsStoreGet($idx, $caller)
	{
		...
		if ($idx === $this->TbsCurrIdx) {
			...
			if ($caller === false)
				$this->TbsStoreLst[$idx]['src'] = ''; // save memory space
			return $txt;
		} else {
...
If they are blank spaces, line beaks, or other unexpected characters, then you have to check your code in order to avoid them.";
		...
		foreach ($Loc->otbsDim as $tDim) { // template dimensions. They must be sorted in reverse order of location
			if ($tDim !== false) {
				...
				foreach ($edit_lst as $what => $new) {
					...
					if ($adjust == '%') {
						if ($tDim['cpt'] !== false)
							$new = $new * $tDim['cpt']; // apply the coef to Point conversion if any
	...
	{
		if ($PrmVal === true)
			$PrmVal = ''; // TBS set the value to true if no value set, but it is converted into '1'.
		...
		foreach ($el_delete as $info) {
			if ($info['p'] === false) {
				...
				while ($ParentLoc = clsTbsXmlLoc::FindElement($Src, $info['p'], $ParentPos, true)) {
					...
					if ($ModifNbr > 0)
						$ParentLoc->ReplaceSrc($ParentSrc);
	...
	function TbsSheetSlide_DeleteDisplay($id_or_name, $ok, $delete)
	{
		...
		$ok = (boolean) $ok;
		if (!is_array($id_or_name))
			$id_or_name = array($id_or_name);
	...
	function Ext_PrepareInfo($Ext = false)
	{
		...
		if (isset($GLOBAL['_OPENTBS_AutoExt'][$Ext])) {
			...
			if ($this->FileExists('styles.xml'))
				$i['load'] = array('styles.xml'); // styles.xml may contain header/footer contents
			if ($Ext === 'odf')
				$i['br'] = false;
			if ($Ext === 'odm')
				$this->ExtEquiv = 'odt';
			...
			$block_alias = array(
				...
				'tbs:title' => 'text:h',
				// ODT+ODP
				'tbs:section' => 'text:section',
				// ODT
				'tbs:table' => 'table:table',
				// ODT (sheet for ODS)
				'tbs:row' => 'table:table-row',
				// ODT+ODS
				'tbs:cell' => 'table:table-cell',
				// ODT+ODS
			...
			if (($Ext === 'docx') || ($Ext === 'docm')) {
				...
				if ($Ext === 'docm')
					$i['ctype'] = 'application/vnd.ms-word.document.macroEnabled.12';
				...
				if ($Ext === 'xlsm')
					$i['ctype'] = 'application/vnd.ms-excel.sheet.macroEnabled.12';
				...
				if ($Ext === 'pptm')
					$i['ctype'] = 'application/vnd.ms-powerpoint.presentation.macroEnabled.12';
				$this->MsPowerpoint_InitSlideLst();
		...
		if ($set_option && ($block_alias !== false))
			$TBS->SetOption('block_alias', $block_alias);

		$this->ExtInfo = $i;
		if ($this->ExtEquiv === false)
			$this->ExtEquiv = $Ext;
		if ($this->ExtType === false)
			$this->ExtType = $Frm;
	...
	{
		if (strpos(',odt,ods,odg,odf,odp,odm,ott,ots,otg,otp,', ',' . $Ext . ',') !== false)
			return 'odf';
		if (strpos(',docx,docm,xlsx,xlsm,pptx,pptm,', ',' . $Ext . ',') !== false)
			return 'openxml';
		if (!$Search)
			return false;
	...
	function XML_DeleteColumnElements(&$Txt, $Tag, $SpanAtt, $ColLst, $ColMax)
	{
		...
		while ($Continue && ($Loc = clsTbsXmlLoc::FindElement($Txt, $Tag, $ColPos, true))) {
			...
			$KeepQty = 0;
			for ($i = 1; $i <= $ColQty; $i++) {
				if (array_search($ColNum + $i, $ColLst) === false)
					$KeepQty++;
			}
			if ($KeepQty == 0) {
				// delete the tag
	...
	function XML_GetNextCellLoc(&$SheetLoc, $Range, $PrevLoc, $RowEl, $CellEl, $AttRowR, $AttCellR, $AddMissing)
	{
		...
		while ($currColOk && ($currCol < $targetCol)) {
			if ($rowLoc === false)
				return $this->RaiseError("No parent row.");
			if (isset($rowLoc->CellLst[$currCol]))
				return $this->RaiseError("This repeated row should have been previsouly catched.");
			if ($debug)
			...
			if (($cellLoc !== false) && ($cellLoc->RepeatIdx < $cellLoc->RepeatMax)) {
				...
				if ($cellLoc === false) {
					if ($debug)
						echo "FAIL, rowLoc = " . $rowLoc->GetSrc();
					$currColOk = false;
				} else {
					if ($debug)
						echo "ok found";
	...
	function OpenXML_Rels_AddNewRid($DocPath, $TargetDir, $FileName)
	{
		...
		if (isset($o->RidLst[$Target]))
			return $o->RidLst[$Target];
	...
	function OpenXML_CTypesPrepareExt($FileOrExt, $ct = '')
	{
		...
		if (isset($lst[$ext]) && ($lst[$ext] !== ''))
			return;

		if (($ct === '') && isset($this->ExtInfo['pic_ext'][$ext]))
			$ct = 'image/' . $this->ExtInfo['pic_ext'][$ext];
	...
	function OpenXML_MapInit()
	{
		...
		while (($p = strpos($Txt, '<', $p + 1)) !== false) {
			$pe = strpos($Txt, '>', $p);
			if ($pe === false)
				return; // syntax error in the XML
			...
			if ($pi !== false) {
				...
				if ($pi !== false) {
					...
					if ($Name[0] == '/')
						$Name = substr($Name, 1); // fix the file path
					if (!isset($Map[$ShortType]))
						$Map[$ShortType] = array();
					$Map[$ShortType][] = $Name;
				}
			}
	...
	function OpenXML_MapGetFiles($ShortTypes)
	{
		...
		if (is_string($ShortTypes))
			$ShortTypes = array($ShortTypes);
		$res = array();
	...
	function OpenXML_ChartSeriesFound(&$Txt, $SeriesNameOrNum, $OnlyBounds)
	{
		...
		if (is_numeric($SeriesNameOrNum)) {
			...
			if ($p === false)
				return "Number of the series not found.";
		} else {
			...
			if ($p === false)
				return "Name of the series not found.";
			$p++;
		}
		...
		for ($i = 1; $i <= 2; $i++) {
			...
			if ($p1 === false)
				return ($i == 1) ? "categories or values not found." : "categories not found, check the chart to add categories.";
			...
			*/
			if ($p2 === false)
				return "Neither Literal nor Cached data is found for categories or values.";
			$p2 = $p2 - 7;
			$res['point' . $i . '_p'] = $p1;
	...
	function OpenXML_ChartGetInfoFromFile($idx, $Txt = false)
	{
		...
		if ($o->ChartLst === false) {
			...
			if ($Txt === false)
				$Txt = $this->TbsStoreGet($idx, 'OpenXML_ChartGetInfoFromFile');
	...
	function OpenXML_SharedStrings_GetVal($id)
	{
		...
		if ($this->OpenXmlSharedStr === false)
			$this->OpenXML_SharedStrings_Prepare();
	...
	function OpenMXL_GarbageCollector()
	{
		...
		if ((count($this->IdxToCheck) == 0) && (count($this->OtbsSheetSlidesDelete) == 0))
			return;
		...
		foreach ($rels as $n => $idx) {
			...
			foreach ($pictures as $i => $info) {
				if (strpos($txt, $info['short'] . '"') !== false)
					$pictures[$i]['nbr']++;
			}
		}
		...
		foreach ($pictures as $info) {
			if ($info['nbr'] == 0)
				$this->FileReplace($info['idx'], false);
		}
	...
	function MsExcel_ChangeCellType(&$Txt, &$Loc, $Ope)
	{
		...
		if (($Ope === 'xlsxString') || ($Ope === 'tbs:string'))
			return true;
		...
		if (($p_is1 === false) || ($p_is1 > $t1))
			return false; // error in the XML structure

		$is2 = '</is>';
		$p_is2 = strpos($Txt, $is2, $p_is1);
		if (($p_is2 === false) || ($p_is2 > $t1))
			return false; // error in the XML structure
	...
	function MsExcel_RangeNamesInit()
	{
		...
		if ($this->OtbsSheetRangeNames !== false)
			return;
	...
	function MsPowerpoint_SlideDebug($nl, $sep, $bull)
	{
		...
		if (count($this->OpenXmlSlideLst) == 0)
			echo $bull . "(none)";
	...
	function MsWord_CleanDuplicatedLayout(&$Txt)
	{
		...
		while (($wro_p = $this->XML_SearchTagForward($Txt, $wro, $wro_p)) !== false) { // next <w:r> tag
			...
			if ($nb > 0) {
				$with_space = false;
				if (substr($Txt, $wtc_p - 1, 1) === ' ')
					$with_space = true;
				$p_end = strpos($Txt, '>', $wto_p); // first char of the text
				if (substr($Txt, $p_end + 1, 1) === ' ')
					$with_space = true;
	...
	function MsWord_GetPage($Tag, $Txt, $Pos, $Forward, $LevelStop)
	{
		...
		$p = $loc->PosBeg;
		if ($Forward)
			$p--; // if it's forward, we stop the block before the paragraph with page-break
		return $p;
	...
	function MsWord_InitHeaderFooter()
	{
		...
		if ($this->MsWord_HeaderFooter !== false)
			return;
	...
	function OpenDoc_ManifestChange($Path, $Type)
	{
		...
		if (($Type === '') && (substr($Path, 0, 9) === 'Pictures/')) {
			...
			if ($p !== false) {
				...
				if (isset($this->ExtInfo['pic_ext'][$ext]))
					$Type = 'image/' . $this->ExtInfo['pic_ext'][$ext];
			}
		}
		...
		foreach ($this->OpenDocManif as $Path => $Type) {
			...
			if ($Type === false) {
				...
				if ($p !== false) {
					...
					$p2 = strpos($Txt, '>', $p);
					if (($p1 !== false) && ($p2 !== false))
						$Txt = substr($Txt, 0, $p1) . substr($Txt, $p2 + 1);
				}
			} else {
	...
	function OpenDoc_ChangeCellType(&$Txt, &$Loc, $Ope, $IsMerging, &$Value)
	{
		...
		if (isset($new_atts['attcurr']) && isset($Loc->PrmLst['currency']))
			$xLoc->ReplaceAtt('office:currency', $Loc->PrmLst['currency'], true);
		...
		if ($IsMerging) {
			...
			if (isset($new_atts['frm']))
				$Loc->PrmLst['frm'] = $new_atts['frm'];
		}
			...
			while ($loc = clsTinyButStrong::f_Xml_FindTag($Txt, 'style:style', true, $p, true, false, true, false)) {
				...
				if (isset($loc->PrmLst['style:name'])) {
					...
					if (isset($styles_to_edit[$name])) {
						...
						if ($p === false)
							return; // bug in the XML contents
						$p = $p + $tag_close_len;
						$src = substr($Txt, $loc->PosBeg, $p - $loc->PosBeg);
						// add the attribute, if missing
						if (strpos($src, ' ' . $att_display . '="') === false)
							$src = str_replace('<' . $tag_property . ' ', '<' . $tag_property . ' ' . $att_display . '="' . $yes_display . '" ', $src);
						// add new styles
	...
	function OpenDoc_StylesInit()
	{
		...
		if ($idx !== false) {
			...
			if ($Txt == !false)
				$this->OpenDoc_StylesFeed($Styles, $Txt);
		}
		...
		foreach ($Styles as $n => $s) {
			if (($s->parentName !== false) && isset($Styles[$s->parentName]))
				$Styles[$s->parentName]->childs[$s->name] = &$s;
		}
	...
	function OpenDoc_StylesFeed(&$Styles, $Txt)
	{
		...
		while ($loc = clsTbsXmlLoc::FindElement($Txt, 'style:style', $p)) {
			...
			$o->ctrl = false;
			$src = $loc->GetSrc();
			if (strpos($src, ' fo:break-before="page"') !== false)
				$o->pbreak = 'before';
			if (strpos($src, ' fo:break-after="page"') !== false)
				$o->pbreak = 'after';
			if ($o->name !== false)
				$Styles[$o->name] = $o;
			$p = $loc->PosEnd;
	...
	function OpenDoc_ChartFind($ChartRef, &$Txt, $ErrTitle)
	{
		...
		if ($this->OpenDocCharts === false)
			$this->OpenDoc_ChartInit();
	...
	function OpenDoc_ChartChangeSeries($ChartRef, $SeriesNameOrNum, $NewValues, $NewLegend = false)
	{
		...
		if (is_numeric($SeriesNameOrNum)) {
			...
			foreach ($series as $idx => $s) {
				if (($s_info === false) && ($s['name'] == $SeriesNameOrNum))
					$s_info = &$series[$idx];
			}
		}
		if ($s_info === false)
		...
		if ($NewLegend !== false)
			$this->OpenDoc_ChartRenameSeries($Txt, $s_info, $NewLegend);
		...
		if (!is_array($NewValues)) {
			$data = array();
			if ($NewValues === false)
				$this->OpenDoc_ChartDelSeries($Txt, $s_info);
				...
				if ($elCell = clsTbsXmlLoc::FindElement($elRow, 'table:table-cell', $p_cell)) {
					if ($i == $col_cat) {
						...
						if ($s_use_cat) {
							...
							$val_idx = $i - $s_col_deb;
							if ($data_r === false)
								$data_r = array_shift($data); // (may return null) delete the row in order to keep only unused
							if ((!is_null($data_r)) && isset($data_r[$val_idx]))
								$x = $data_r[$val_idx];
						}
		...
		while ($drEl = clsTbsXmlLoc::FindElement($Txt, 'draw:frame', $p)) {
			...
			if ($objEl) { // Picture have <draw:frame> without <draw:object>
				...
				if ($href) {
					...
					if (substr($href, 0, 2) == './')
						$href = substr($href, 2);
					if (is_string($img_href) && (substr($img_href, 0, 2) == './'))
						$img_href = substr($img_href, 2);
	...
	function OpenDoc_ChartFindSeries(&$chart, $Txt)
	{
		...
		while ($elSeries = clsTbsXmlLoc::FindElement($Txt, 'chart:series', $p)) {
			...
			$series[$s_idx] = array(
				...
				'val_col' => false,
				// column index for the values
				'used_cols' => false,
				// list of column indexes in the table that are used by the series
				'used_refs' => $used_refs,
				// list of ref usde by the series, (0] is always for values
				'label_ref' => $label_ref,
		...
		if ($elTbl === false)
			return $this->RaiseError("(ChartFindSeries) : unable to found the local table in the chart " . $this->_ChartCaption . ".");
		...
		if ($elRow === false)
			return $this->RaiseError("(ChartFindSeries) : unable to found the header row in the chart " . $this->_ChartCaption . ".");
		...
		if ($elRow === false)
			return $this->RaiseError("(ChartFindSeries) : unable to found the first data row in the chart " . $this->_ChartCaption . ".");
	...
	function OpenDoc_FirstColIdx($RangeRef, $LocTblName, $def_col_idx)
	{
		...
		if ($col_num === false)
			return $this->RaiseError('(FirstColIdx) Reference of cell \'' . $RangeRef[0] . '\' cannot be recognized.');
